...
class _CountryFlagIcon extends StatelessWidget {
  ...
  // 与 NTP 小部件 (Icons.access_time) 及标准 24dp Material Icon 视觉重量与尺寸 1:1 像素级精确对齐：
  // 1. 描边线宽提升至 2.0dp，与标准 Material Icon 笔画粗细完全一致，彻底消除线条偏细感；
  // 2. 外圆直径严格对齐 Material 24dp 关键帧外圆 20.0dp（内径 16.0dp）；
  // 3. 描边完全从国旗外边缘向外扩张绘制（往外扩），国旗尺寸设为 16.2dp，仅保留 0.1dp 极微重合封死抗锯齿，
  //    100% 杜绝白边缝隙的同时，绝对不遮挡/不侵占国旗图案内容；
  // 4. 外层槽位 24x24dp，与标准 Material Icon 占位及间距无缝对齐。
  static const double _totalDiameter = 20.0;
  static const double _strokeWidth = 2.0;
  static const double _flagDiameter = 16.2;

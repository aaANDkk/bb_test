...
class _CountryFlagIcon extends StatelessWidget {
  ...
  // 1. 整体最外径锁定 17.5dp，线宽 1.2dp；
  // 2. 纯净独立矢量 Stroke 硬件绘制：彻底根除复合图层造成的边缘发虚与粗糙感，线条如原生 Icon 般极致锐利纯净；
  // 3. 国旗外径设为 16.5dp，深入黑边中心线（16.3dp）下方 0.7dp 处，利用黑边实体不透明区域彻底封死抗锯齿缝隙，在 Windows/高分屏各缩放下绝对 0 空隙 0 白边；
  // 4. 黑边内孔直径为 15.1dp，国旗内部所有徽标、星徽、图案 100% 完整舒展，不受任何遮挡；
  // 5. 外层总宽 21.5dp (左右各留 2.0dp 边距)，高度 24.0dp，与标准 24dp Material Icon 视觉边缘严格对齐。
  static const double _totalDiameter = 17.5;
  static const double _strokeWidth = 1.2;
  static const double _flagDiameter = 16.5;

...
class _CountryFlagIcon extends StatelessWidget {
  ...
  // 1. 整体最外径锁定 17.6dp，外包边线宽 1.2dp（内径 15.2dp）；
  // 2. 国旗直径设为 16.8dp（比全幅 17.6dp 适度收敛），图案等比向内聚拢，大幅减少边缘内容被黑边遮挡的面积；
  // 3. 顶层矢量 Stroke 仅覆盖国旗外沿 0.8dp，既充分舒展国旗画面，又足以彻底封死抗锯齿（绝对 0 缝隙、0 白边）；
  // 4. 外层总宽 21.6dp (左右各留 2.0dp 边距)，高度 24.0dp，与标准 24dp Material Icon 视觉边缘严格对齐。
  static const double _totalDiameter = 17.6;
  static const double _strokeWidth = 1.2;
  static const double _flagDiameter = 16.8;
